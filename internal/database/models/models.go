...
type User struct {
	...
	PasswordHash         string         `gorm:"size:255" json:"-"` // nullable: empty for OIDC-only accounts
	...
	OIDCSubject          string         `gorm:"column:oidc_subject;size:255;index" json:"-"`                                      // OIDC "sub" claim; set on first OIDC login
